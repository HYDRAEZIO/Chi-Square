...
var _download_csv_friedman_test = function (_dimension) {
    ...
    while (true) {
        ...
        // Find the smallest value in the row and column first
        for (var _i = 0; _i < _attr_list.length; _i++) {
            var _min_x = null;
            var _min_y = null;
            var _min_freq = null;
            ...
            if (_min_freq === null) {
                console.log("No suitable solution found");
        ...
        for (var _y_name in _ct_json[_x_name]) {
            ...
            if (_tbody.find('.variable_y[value="' + _y_name + '"]').length === 0) {
                ...
                if (_tr.find("th").length > 1) {
                    // Indicates that the first column already has data
...
var _draw_result_table = function () {
    ...
    var _cross_table = $('<div class="cross-table">crosstab<table border="1" cellpadding="0" cellspacing="0">'
        + '<thead>'
            + '<tr class="x-var-tr"><th colspan="3" rowspan="2"></th><th class="x-var-name"></th><th rowspan="2" valign="bottom">' + 'column sum' + '</th></tr>'
            + '<tr class="x-vars-tr"></tr></thead>'
        + '<tbody></tbody>'
        + '<tfoot>'
            + '<tr class="x-sum num-tr"><th rowspan="5" colspan="2" align="left" valign="top">' + 'row sum' + '</th><th align="left" valign="top">' + 'number' + '</th></tr>'
            + '<tr class="x-sum exp-tr"><th align="left" valign="top">' + 'expected number' + '</th></tr>' 
            + '<tr class="x-sum per y-per-tr"><th align="left" valign="top">' + '(<span class="y-var-name"></span>)percentage within column' + '</th></tr>'
            + '<tr class="x-sum per x-per-tr"><th align="left" valign="top">' + '(<span class="x-var-name"></span>)percentage within the column' + '</th></tr>'
            + '<tr class="x-sum per per-tr"><th align="left" valign="top">' + 'Overall percentage' + '</th></tr>'
        + '</tfoot>'
        + '</table></div>');
...
// Then draw the y variable
    ...
    $("#contingency_table input.variable_y").each(function (_i, _input) {
        ...
        var _tr_num = $('<tr y_var="' + _name + '" class="num-tr"><th rowspan="' + _rowspan + '" class="bottom-border-thin" valign="top" align="left">' + _name + '</th>'
                + '<th valign="top" align="left">' + 'number' + '</th></tr>').appendTo(_tbody);
        var _tr_exp = $('<tr y_var="' + _name + '" class="exp-tr"><th valign="top" align="left">' + 'expected number' + '</th></tr>').appendTo(_tbody);
        if (_is_display_percent()) {
            var _tr_y_per = $('<tr y_var="' + _name + '" class="y-per-tr"><th valign="top" align="left">' + '(<span class="y-var-name"></span>)percentage within column' + '</th></tr>').appendTo(_tbody);
            var _tr_x_per = $('<tr y_var="' + _name + '" class="x-per-tr"><th valign="top" align="left">' + '(<span class="x-var-name"></span>)percentage within the column' + '</th></tr>').appendTo(_tbody);
            var _tr_global = $('<tr y_var="' + _name + '" class="global-tr"><th valign="top" align="left">' + 'Overall percentage' + '</th></tr>').appendTo(_tbody);
        }
        var _tr_residual = $('<tr y_var="' + _name + '" class="residual-tr"><th valign="top" align="left">' + 'residual' + '</th></tr>').appendTo(_tbody);
        var _tr_std_residual = $('<tr y_var="' + _name + '" class="std-residual-tr"><th valign="top" align="left">' + 'Standardized residuals' + '</th></tr>').appendTo(_tbody);
        var _tr_adj_residual = $('<tr y_var="' + _name + '" class="adj-residual-tr bottom-border-thin"><th valign="top" align="left">' + 'Adjusted residuals' + '</th></tr>').appendTo(_tbody);
        
        // -----------
        
        if (_y_vars_count === 0) {
            // Insert the name of the y variable
...
var _draw_num_cell = function () {
    ...
    if (_is_sum_zero_cell_existed === true) {
        ...
        $('<div>because「' + _zero_var_list.join("」、「") + '」The total number is 0，Unable to perform contingency table analysis，It is recommended to delete the above variables and then analyze。</div>').appendTo(_result);
        _cross_table.hide();
        return;
    }
...
var _draw_contingency_table_analyze_result = function (_chi_squared, _yates_chi_squared) {
    ...
    var _title_container = $('<div>Chi-square test result：</div>').appendTo(_result);
    ...
    _button.click(function () {
        ...
        _text = _text.text();
        _text = "Contingency table analysis results display：" + _text + "Contingency table analysis ends。";
        console.log(_text);
        ...
        */
        ...
        _text = "Chi-square test result display。" + _text + "Chi-square test is over。";
...
//                }, _speak_list[_i].length * 1000 * 0.3 );
    ...
    var _sig_pass = '，achieve<span class="skip">α = </span> 0.05salient level of ，So reject the null hypothesis，Accept the opposing hypothesis.'
                    + 'express<span class="speak">「' + _y_var_name + '」different pairs of「' + _x_var_name + '」have a significant impact。</span></li>';
    var _sig_not_pass = '，not reached<span class="skip">α = </span> 0.05salient level of，Therefore, the null hypothesis cannot be rejected。'
                    + 'express<span class="speak">「' + _y_var_name + '」different pairs of「' + _x_var_name + '」no significant effect。</span></li>';
...
//        }
        ...
        var _text = 'Two-tailed probability value of Fishers correct probability test<span class="skip" alt="for"> P value = </span> ' + precision_string(_p, 3) + ' 。';
        if (Math.abs(_p) < 0.05) {
        ...
        var _text = 'Chi-square test statistic using Yeps continuity correction<span class="skip" alt="for">χ<sup>2</sup> = </span>' + precision_string(_chi_squared, 3) 
                    + ' ，<span class="skip" alt="The probability is">P value = </span>' + precision_string(_p, 3) + ' ';
        if (Math.abs(_p) < 0.05) {
            _chi_squared_container.append('<li>' + _text + _sig_pass + '</li>');
            _has_sig = true;
        }
        ...
        var _text = 'Chi-square test statistic<span class="skip" alt="for">Χ<sup>2</sup> = </span> ' + precision_string(_chi_squared, 3) 
                    + ' ，<span class="skip" alt="The probability is">P value = </span>' + precision_string(_p, 3) + ' ';
        if (Math.abs(_p) < 0.05) {
            _chi_squared_container.append('<li>' + _text + _sig_pass + '</li>');
            _has_sig = true;
    ...
    if (_has_sig === true || _DEBUG.force_sig_pass) {
        ...
        var _cramer_v_desc = "，<span class='speak'>belong to irrelevant。</span>";
        if (_cramer_v === 1) {
            _cramer_v_desc = "，<span class='speak'>fully related。</span>";
        }
        else if (_cramer_v > 0.7) {
            _cramer_v_desc = "，<span class='speak'>are highly correlated。</span>";
        }
        else if (_cramer_v > 0.4) {
            _cramer_v_desc = "，<span class='speak'>is moderately correlated。</span>";
        }
        else if (_cramer_v > 0.1) {
            _cramer_v_desc = "，<span class='speak'>low correlation。</span>";
        }
        var _cramer_v_li = $('<li><span class="speak">「' + _y_var_name + '」and「' + _x_var_name + '」'
            + '</span>the correlation coefficientCramer\'s V value<span class="skip" alt="for">(between 0 and 1)</span>for ' + precision_string(_cramer_v, 3) + ' ' + _cramer_v_desc + '</li>')
            .appendTo(_chi_squared_container);
        
        // -----------------------------------
        var _tau_container = $('<li><span class="skip" alt="then proceed"></span>GoodmanandKruskalAnalysis of the prediction coefficient tau value：<ul></ul></li>')
                .appendTo(_chi_squared_container);
        ...
        $('<li> with「' + _y_var_name + '」to predict「' + _x_var_name + '」The correct ratio is' 
        ...
        $('<li>以「' + _x_var_name + '」to predict「' + _y_var_name + '」The correct ratio is' 
        ...
        if (_sig_cell.length > 0) {
            var _cell_container = $('<li><span class="skip" alt="last"></span>fine-grained statistical analysis：<ul></ul></li>');
            ...
            _sig_cell.each(function (_i, _td) {
                ...
                var _text = '<span class="speak">「' + _y_var + '」中「' + _x_var +'」</span><span class="speak" alt="of"></span>The adjusted residual is' + _adj_residual + '，express<span class="speak">Significant number of observations';
                if (_adj_residual > 0) {
                    _text += "higher than expected。</span>";
                }
                else {
                    _text += "lower than expected。</span>";
                }
...
var _calc_fisher_exact_test = function () {
    ...
    // ---------------------
// Both p1 and p2 are fixed, first calculate p1 and p2
